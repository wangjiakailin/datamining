...
outputUrlListFile 	= "urlList/url_latest_yanbao.txt"
...
class jobSpider(scrapy.Spider):
    ...
    def parse1(self, response):
        ...
        #1. get the pageCount        
        pageStr = sel.xpath('//div[@class="pager"]/span[@class="pagernums"]/@data-pager').extract()
        pagelist = str(pageStr[0]).split('|')
        # pagelist = utstr.split('|')
        pageStr = pagelist[1]
        pageCount = getPageCount(pageStr)

        
        for pageId in range(0, pageCount+1):
            url = 'http://guba.eastmoney.com/list,'+str(stockId)+',2,f_'+str(pageId)+'.html'
            #print "########url: "+str(url)
